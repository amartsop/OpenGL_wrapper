...
// Assign mesh (for geometry file)
void Mesh::assign(Geometry *geometry, GLenum draw_type)
{
    // Draw type 
    m_draw_type = draw_type;

    // Assign vertices to indexed model
    for (size_t i = 0; i < geometry->get_vertices_number(); i++)
    {
        m_model.positions.push_back(geometry->get_vertex_positions().at(i));
        m_model.texCoords.push_back(geometry->get_texture_coordinates().at(i));
        m_model.normals.push_back(geometry->get_normal_coordinates().at(i));
    }

    // Assign indices to indexed model
    for (size_t i = 0; i < geometry->get_indices_number(); i++)
    {
        m_model.indices.push_back(geometry->get_indices().at(i));
    }
